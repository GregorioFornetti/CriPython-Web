...
function criar_elementos_modo_CEL() {
    ...
    tri_barra.addEventListener('click', () => {
        ...
        if (!div_opcoes) {  // Usuário clicou quando a barra ainda não estava aberta.
            ...
            // Criando botão "login/perfil"
            let div_botao_login_perfil = document.createElement('div')
            div_botao_login_perfil.className = 'container-botao-sub-barra-cel'

            let botao_login_perfil = retorna_botao_login_perfil()
            botao_login_perfil.className = 'texto-titulos-sub-barra-cel'
            botao_login_perfil.addEventListener('click', limpar_sub_barra_CEL)
            div_botao_login_perfil.append(botao_login_perfil)
            ...
            sub_barra.append(div_botao_login_perfil)
...
window.addEventListener('resize', () => {
    if (innerWidth > 900 && !modo_PC) {
        limpar_elementos_CEL()
        limpar_containers()
        criar_elementos_modo_PC()
        modo_PC = true
    } 
    else if (innerWidth <= 900 && modo_PC) {
        limpar_elementos_PC()
        limpar_containers()
        criar_elementos_modo_CEL()
        modo_PC = false
...
function colocar_botoes_no_sub_menu_CEL(lista_botoes, div_botoes) {
    lista_botoes.forEach((botao) => {
        ...
        botao.addEventListener('click', limpar_sub_barra_CEL)
